(() => {
  ...
  function mount({ host, getCategories, onCommit }) {
    ...
    const render = () => {
      ...
      host.querySelectorAll('[data-local-ocr-row]').forEach(node => node.querySelectorAll('[data-field]').forEach(input => input.oninput = input.onchange = () => {
